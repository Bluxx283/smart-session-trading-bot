...
if st.session_state.active_tab == "📊 Dashboard":
    ...
    m4.metric("Broker Status", "Online" if st.session_state.broker_connected else "Demo Mode")
    ...
    if len(st.session_state.positions) > 0:
        ...
        st.info("No active trades recorded. Use 'Open Charts' or 'Open Gateway' to route an execution order.")
...
# ==========================================
    ...
    with col_s2:
        inst_select = st.selectbox("Trading Instrument", list(MARKET_UNIVERSE[cat_select].keys()))
...
with tab_quant:
    ...
    if active_df is not None and len(active_df) >= 20:
        ...
        active_df['z_score'] = (active_df['Close'] - rolling_m) / (rolling_s + 1e-8)
        active_df['vol_surge'] = active_df['Volume'] / (active_df['Volume'].rolling(20).mean() + 1e-8)
        ...
        is_pivot = (clean_df['High'] == clean_df['High'].rolling(5, center=True).max()) | \
                   (clean_df['Low'] == clean_df['Low'].rolling(5, center=True).min())
        clean_df['anomaly'] = raw_anomaly & is_pivot & ((clean_df['z_score'].abs() > 1.8) | (clean_df['vol_surge'] > 2.0))
        ...
        if not anomalies.empty:
            fig.add_trace(go.Scatter(
                x=anomalies.index, y=anomalies['High'] * 1.003,
                mode='markers',
        ...
        fig.update_layout(
            template="plotly_dark", height=540, margin=dict(l=10, r=10, t=10, b=10),
            dragmode="pan", xaxis_rangeslider_visible=False, paper_bgcolor="#0b0e14", plot_bgcolor="#0b0e14"
        )
        st.plotly_chart(fig, use_container_width=True, config={"scrollZoom": True})

        st.markdown("#### 🔬 Detected Session Anomalies Log")
        if not anomalies.empty:
            anomaly_sheet = anomalies[['Close', 'z_score', 'vol_surge', 'rolling_vol']].copy()
            anomaly_sheet.columns = ['Price ($)', 'Z-Score (σ)', 'Volume Surge Ratio', 'Rolling Volatility']
...
with col_c1:
    c_bal = st.number_input("Portfolio Balance ($)", value=float(st.session_state.account_balance), step=1000.0)
...
with col_g1:
    ...
    target_broker = st.selectbox("Broker Gateway", ["MetaTrader 5 (MT5)", "Zerodha (Kite)", "Binance Futures", "Interactive Brokers"])
    ...
    trade_lots = st.number_input("Volume (Lots)", min_value=0.01, max_value=20.0, value=1.0, step=0.1)
