...
"Portfolio Balance ()",value=float(st.session 
s
​
 tate.account 
b
​
 alance),step=1000.0,)c 
r
​
 isk 
p
​
 ct=st.slider("RiskTolerance(c 
r
​
 isk 
u
...
p
​
 ct)/100.0st.metric("TotalCapitalatRisk",f"{c_risk_usd:,.2f}")

with col_c2:
c_entry = st.number_input("Entry Price ()",value=float(global 
p
​
 rice))c 
s
...
i
​
 nput("StopLoss()", value=float(global_price * 0.992))
